...
static const unsigned int tex_indices[12][3] = {
    { 12, 13, 11 }, { 12, 11, 10 },
    { 7, 8, 11 }, { 7, 11, 10 },
    { 4, 8, 9 }, { 4, 9, 5 },
    { 3, 2, 6 }, { 3, 6, 7 },
...
void paintGL() {
    ...
    glRotatef(theta, 2.0f, 0.0f, 0.0f);
    ...
    for (int face = 0; face < 6; face++) {
        ...
        for (int i = 0; i < 3; i++) {
            glTexCoord2fv(texcoords[tex_indices[face * 2 + 0][i]]);
            glVertex3fv(positions[indices[face * 2 + 0][i]]);
        }
        
        for (int i = 0; i < 3; i++) {
            glTexCoord2fv(texcoords[tex_indices[face * 2 + 1][i]]);
