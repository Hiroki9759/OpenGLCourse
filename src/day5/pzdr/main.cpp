...
static const char *WIN_TITLE = "OpenGL Final Course";     // ウィンドウのタイトル
...
// 頂点オブジェクト
struct Vertex {
    Vertex(const glm::vec3 &position_, const glm::vec2 &tex_,const int &dropId1_)
        : position(position_)
        , tex(tex_) 
        , dropId1(dropId1_){
    }
...
struct Drop{
  glm::mat4 modelMat, acRotMat ,acTransMat ,acScaleMat;
 float acScale = 1.0f;
...
void initVAO() {
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 3; j++) {
            Vertex v(positions[indices1[i][j]],tex_pos[f] +tex_offset[j + i * 3],e);
    ...
    glVertexAttribPointer(2, 1, GL_INT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, dropId1));
    // 頂点番号バッファの作成
...
void initializeGL() {
    ...
    initTexture();
    // カメラの初期化
    projMat = glm::perspective(45.0f, (float)WIN_WIDTH / (float)WIN_HEIGHT, 0.1f, 100.0f);

    viewMat = glm::lookAt(glm::vec3(0.1f, 0.1f, 5.0f),   // 視点の位置
                          glm::vec3(0.1f, -0.5f, -5.1f),   // 見ている先
                          glm::vec3(0.0f, 0.0f, 1.0f));  // 視界の上方向
    
    for(int d = 0; d < 64;d++){
    drop[d].initialize();
    drop[d].modelMat = glm::translate(drop[d].modelMat, glm::vec3((d%8)*0.25f, (d/8)*0.25f, 0.0f));
    }
...
void paintGL() {
	...
	glBindTexture(GL_TEXTURE_2D, textureId);
    GLuint uid= glGetUniformLocation(textureId, "u_texture");
    glUniform1i(uid, 0);
    //立方体を描画
    for(int l = 0;l < 64 ; l++){
    f = rand() % 7;
    GLuint uid = glGetUniformLocation(programId, "u_selectDrop");
    glUniform1i(uid,l);
    drop[l].draw();
    }
...
void updateRotate() {
    ...
    const glm::mat4 c2oMat = glm::inverse(viewMat * drop[selectObject].modelMat);
    // オブジェクト座標における回転軸
    ...
    drop[selectObject].acRotMat = glm::rotate((float)(4.0 * angle), rotAxisObjSpace) * drop[selectObject].acRotMat;
}

void updateTranslate() {
    // オブジェクト重心のスクリーン座標を求める
    
     glm::vec4 gravityScreenSpace = (projMat * viewMat * drop[selectObject].modelMat) * glm::vec4(gravity.x, gravity.y, gravity.z, 1.0f);
    ...
    const glm::mat4 s2oMat = glm::inverse(projMat * viewMat * drop[selectObject].modelMat);
...
void updateScale() {
    ...
    drop[selectObject].acScaleMat = glm::scale(glm::vec3(drop[selectObject].acScale, drop[selectObject].acScale, drop[selectObject].acScale));
...
void updateMouse() {
    ...
    case ARCBALL_MODE_SCALE:
        drop[selectObject].acScale += (float)(oldPos.y - newPos.y) / WIN_HEIGHT;
...
void wheelEvent(GLFWwindow *window, double xpos, double ypos) {
    ...
    drop[selectObject].acScale += ypos / 10.0;
    updateScale();
}
