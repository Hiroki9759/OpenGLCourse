...
static const char *WIN_TITLE = "OpenGL Course Finel";     // ウィンドウのタイトル
...
struct Operate{
 ...
 Operate(){
   ...
   // acScaleMat = glm::mat4(1.0);
    ...
    void draw(){
        glm::mat4 mvpMat =  modelMat * acTransMat /* acRotMat * acScaleMat*/;
...
void initVAO() {
    ...
    for (int k = 0; k < 2; k++) {
        for (int j = 0; j < 3; j++) {
            Vertex v(positions[drops[k][j]], tex_pos[0]+tex_offset[j+k*3]);
...
void initializeGL() {
   ...
   // projMat = glm::perspective(45.0f, (float)WIN_WIDTH / (float)WIN_HEIGHT, 0.1f, 1000.0f);
    ...
    // viewMat = glm::lookAt(glm::vec3(0.0f, 1.0f, 5.1f),   // 視点の位置
...
void updateRotate() {
    ...
    const glm::mat4 c2oMat = glm::inverse( operate[selectObject].modelMat);
