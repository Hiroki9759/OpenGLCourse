...
// 頂点オブジェクト
struct Vertex {
    Vertex(const glm::vec3 &position_, const glm::vec2 &tex_/*,const float &dropId_ */)
        : position(position_)
        , tex(tex_) 
        /*, dropId(dropId_)*/
        {
...
struct Drop{
    ...
    void initialize(int pos){
       ...
       // modelMat = glm::translate(modelMat, glm::vec3(x*0.25f, y*0.25f, 0.0f));
    } 
    void draw(){
        ...
        uid= glGetUniformLocation(textureId, "u_texture");
        glUniform1i(uid, 0);
        uid = glGetUniformLocation(programId, "u_selectMode");
        glUniform1i(uid, selectMode);
        uid = glGetUniformLocation(programId, "u_selectDrop");
        glUniform1i(uid, selectDrop ); 
...
void initVAO() {
    ...
    for(int k = 0; k < 2; k++){
        for (int j = 0; j < 3; j++) {
            Vertex v(positions[indices1[k][j]], tex_pos[0] +tex_offset[j + k * 3]);
    ...
    //             for(int k = 0;k<2;k++){
    //                 for (int j = 0; j < 3; j++) {
    //                     Vertex v(positions[pzdrfaces[][j]], tex_pos[l] + tex_offset[j + k * 3],i + 1);
    //                     vertices.push_back(v);
    //                     indices.push_back(idx++);
    ...
    //  glVertexAttribPointer(2, 1, GL_INT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, dropId));
...
void initializeGL() {
    ...
    initTexture();
     ...
     for(int i = 0; i < 64 ; i++){
        drops[i].initialize(i);
        drops[i].modelMat = glm::translate(drops[i].modelMat, glm::vec3((i%8)*0.25f, (i/8)*0.25f, 0.0f));
    }
...
void updateTranslate() {
    // オブジェクト重心のスクリーン座標を求める
     glm::vec4 gravityScreenSpace = (projMat * viewMat * drops[selectDrop].modelMat) * glm::vec4(gravity.x, gravity.y, gravity.z, 1.0f);
    ...
    const glm::mat4 s2oMat = glm::inverse(projMat * viewMat * drops[selectDrop].modelMat);
    ...
    // オブジェクト空間での平行移動
    drops[selectDrop].acTransMat = glm::translate(drops[selectDrop].acTransMat, transObjSpace);
}
