...
#include <unistd.h>
#include <chrono>
using ms_f = std::chrono::duration<float, std::ratio<1, 1000>>;
...
static const glm::vec2 type_diff[8]={
    glm::vec2(-0.5f,0.75f),glm::vec2(-0.5f,0.5f),glm::vec2(-0.5f,0.25f),glm::vec2(-0.5f,0.0f),
...
// OpenGLの描画関数
void waitmillisec(float millisec){
    auto start = std::chrono::system_clock::now();
    auto end =std::chrono::system_clock::now();
    float dt = 0.0f;
    while(dt < millisec){
        end =std::chrono::system_clock::now();
        dt = std::chrono::duration_cast<ms_f>(end - start).count();
        paintGL();
    }
}
int n = 0;
...
{   
    ...
    for(int i = 55;i >=0;i--)
    {
        ...
        {
            printf("prefalldropup: %d, %d \n",droptypenumber[i+8],i+8);
            printf("prefalldropdown: %d, %d\n",droptypenumber[i],i);
            int save = droptypenumber[i];
            droptypenumber[i] = droptypenumber[i+8];
            droptypenumber[i+8]=8;
            printf("afterfalldropup: %d, %d\n",droptypenumber[i+8],i+8);
            printf("afterfalldropdown: %d, %d\n",droptypenumber[i], i);
...
void keyboardEvent(GLFWwindow *window, int key, int scancode, int action, int mods) {
    ...
    if (action == GLFW_PRESS){
        ...
        std::cout << "just befor vanish\n";
       // waitmillisec(3000.0f);
