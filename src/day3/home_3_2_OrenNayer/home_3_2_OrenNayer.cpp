...
static const char *WIN_TITLE = "Oren-Nayer";     // ウィンドウのタイトル
...
// モデルのファイル
static const std::string OBJECT_FILE = std::string(DATA_DIRECTORY) + "victoria clock.obj";
...
// Gold (参照： http://www.barradeau.com/nicoptere/dump/materials.html)static const glm::vec3 lightPos = glm::vec3(5.0f, 5.0f, 5.0f);
...
void paintGL() {
	...
	// 座標の変換
		glm::mat4 projMat = glm::perspective(45.0f,
			(float)WIN_WIDTH / (float)WIN_HEIGHT, 0.1f, 70.0f);

		glm::mat4 viewMat = glm::lookAt(glm::vec3(3.0f, 3.0f, 15.0f),   //視点の位置
	...
	uid = glGetUniformLocation(programId, "u_lightMat");
	glUniform3fv(uid, 1, glm::value_ptr(lightMat));
