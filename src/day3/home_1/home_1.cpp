...
static int WIN_WIDTH = 500;                   //ウインドウの幅   
static int WIN_HEIGHT = 500;                  //ウインドウの高さ     
static const double PI = 4.0 * std::atan(1.0); //πの定義

static const char *WIN_TITLE = "OpenGL Cource";
// シェーダファイル
...
static std::string BG_VERT_SHADER_FILE = std::string(SHADER_DIRECTORY) + "bgrender.vert";
static std::string BG_FRAG_SHADER_FILE = std::string(SHADER_DIRECTORY) +  "bgrender.frag";
...
static const glm::vec3 bgpositions1[3] = {
	glm::vec3(-1.0f, -1.0f, 0.0f),
	glm::vec3(-1.0f, 3.0f, 0.0f),
	glm::vec3(3.0f, -1.0f, 0.0f)
};

static const glm::vec3 topcol(1.0f, 1.0f, 1.0f);
static const glm::vec3 bottomcol(0.0f, 0.0f, 0.0f);
...
GLuint compileShader(const std::string &filename, GLuint type) {
	...
	code.assign(std::istreambuf_iterator<char>(reader),std::istreambuf_iterator<char>());   // 先頭からファイルサイズ分を読んでコードの変数に格納
...
struct GradationBackground {
	GradationBackground() {
		topcolor = glm::vec3(0.0f, 1.0f, 0.0f);
		bottomcolor = glm::vec3(0.0f, 1.0f, 0.0f);
	}
	GradationBackground(const glm::vec3 &top, const glm::vec3 &bottom)
		: topcolor(top), bottomcolor(bottom) {}
	glm::vec3 topcolor;
	glm::vec3 bottomcolor;

	GLuint bgvaoId;
	GLuint bgvertexBufferId;
	GLuint bgindexBufferId;

	GLuint bgprogramId;

	void initialize() {
		glGenVertexArrays(1, &bgvaoId);
		glBindVertexArray(bgvaoId);

		glGenBuffers(1, &bgvertexBufferId);
		glBindBuffer(GL_ARRAY_BUFFER, bgvertexBufferId);
		glBufferData(GL_ARRAY_BUFFER, sizeof(bgpositions1), bgpositions1, GL_STATIC_DRAW);
		...
		bgprogramId = buildShaderProgram(BG_VERT_SHADER_FILE, BG_FRAG_SHADER_FILE);
	}
	...
	void draw() {
		...
		GLuint topcolLocId = glGetUniformLocation(bgprogramId, "top_color");
		GLuint bottomcolLocId = glGetUniformLocation(bgprogramId, "bottom_color");

		glUniform4f(topcolLocId, topcolor.x, topcolor.y, topcolor.z, 1.0f);
		glUniform4f(bottomcolLocId, bottomcolor.x, bottomcolor.y, bottomcolor.z, 1.0f);
...
void initializeGL() {
	...
	bg = GradationBackground(topcol, bottomcol);
	bg.initialize();
...
void paintGL() {
	...
	glm::mat4 viewMat = glm::lookAt(glm::vec3(3.0f, 4.0f, 5.0f),   
