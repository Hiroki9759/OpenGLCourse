...
static const char *WIN_TITLE = "OpenGL Course4-2";     // ウィンドウのタイトル
...
void initVAO() {
    ...
    for (int i = 0; i < 6; i++) {
        for (int j = 0; j < 3; j++) {
            Vertex v(positions[faces[i * 2 + 0][j]], tex_pos[i] + tex_offset[j],i + 1);
            vertices.push_back(v);
            indices.push_back(idx++);
        }

        for (int j = 0; j < 3; j++) {
            Vertex v(positions[faces[i * 2 + 1][j]],tex_pos[i] + tex_offset[j+3],i + 1);
            vertices.push_back(v);
            indices.push_back(idx++);
        }
    }
