...
void paintGL() {
              ...
              // 1つ目の立方体を描画
              {
                            ...
                            modelMat = glm::rotate(modelMat, theta * 2, glm::vec3(1.0f, 0.0f, 0.0f));
                            glm::mat4 mvpMat = projMat * viewMat * modelMat * op.acTransMat * op.acRotMat* op.acScaleMat;
...
struct operation{
      ...
      void selectObject(GLFWwindow *window, int button, int action, int mods) {
            if (action == GLFW_PRESS) {
            // クリックされた位置を取得
            double px, py;
            glfwGetCursorPos(window, &px, &py);
