...
static const char *WIN_TITLE = "OpenGL Course4-1";     // ウィンドウのタイトル
//πの定義
...
void mouseEvent(GLFWwindow *window, int button, int action, int mods) {
    ...
    if (action == GLFW_PRESS) {
        ...
        if (!isDragging) {
            isDragging = true;
            oldPos = glm::ivec2(cx, cy);                                       
        }
    }
    else {
        isDragging = false;
        newPos = glm::ivec2(px, py);
        if(oldPos.x != newPos.x || oldPos.y != newPos.y) printf("Drug(%s): (%d, %d) →(%d ,%d)\n",button_c, oldPos.x, oldPos.y, newPos.x, newPos.y);
    }
}

void motionEvent(GLFWwindow *window, double px, double py) {
    int action;
    if(!Press) printf("Move: (%d, %d)\n", (int)px, (int)py);
    else printf("Drug(%s): (%d, %d)\n",button_c, (int)px, (int)py);
}
